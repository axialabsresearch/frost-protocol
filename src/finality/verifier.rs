/*!
# Finality Verifier Module

This module provides the core finality verification functionality for the FROST protocol.
It implements various verification strategies with support for rate limiting, caching,
...
2. **Caching Verifier**
   ...
   pub struct CachingVerifier<V: FinalityVerifier> {
       inner: V,
       cache: Arc<RwLock<LruCache<BlockRef, CachedResult>>>,
       // ...
   }
   ```
   - Result caching
...
The verifier system integrates with:
1. Block processing
